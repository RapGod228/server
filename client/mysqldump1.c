...
static my_bool verbose= 0, opt_no_create_info= 0, opt_no_data= 0,
               opt_no_data_med= 1, quick= 1, extended_insert= 1,
               lock_tables= 1, ignore_errors= 0, flush_logs= 0,
               flush_privileges= 0, opt_drop= 1, opt_keywords= 0, opt_lock= 1,
               opt_compress= 0, opt_copy_s3_tables= 0, opt_delayed= 0,
               create_options= 1, opt_quoted= 0, opt_databases= 0,
               opt_alldbs= 0, opt_create_db= 0, opt_lock_all_tables= 0,
               opt_set_charset= 0, opt_dump_date= 1, opt_autocommit= 0,
               opt_disable_keys= 1, opt_xml= 0, opt_delete_master_logs= 0,
               tty_password= 0, opt_single_transaction= 0, opt_comments= 0,
               opt_compact= 0, opt_hex_blob= 0, opt_order_by_primary= 0,
               ...
               opt_logging, opt_drop_trigger= 0, opt_wildcards= 0;
...
static TYPELIB opt_system_types= {array_elements(opt_system_type_values),
                                  "system dump options",
...
const char *compatible_mode_names[]= {"MYSQL323",
                                      "MYSQL40",
                                      "POSTGRESQL",
                                      "ORACLE",
                                      "MSSQL",
...
#define MED_ENGINES                                                           \
  "MRG_MyISAM, MRG_ISAM, CONNECT, OQGRAPH, SPIDER, VP, FEDERATED"
...
static struct my_option my_long_options[]= {
    ...
    {"apply-slave-statements", OPT_MYSQLDUMP_SLAVE_APPLY,
     ...
     "of dump.",
     &opt_slave_apply, &opt_slave_apply, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    ...
    {"copy_s3_tables", OPT_COPY_S3_TABLES,
     "If 'no' S3 tables will be ignored, otherwise S3 tables will be copied "
     "as "
     " Aria tables and then altered to S3",
     &opt_copy_s3_tables, &opt_copy_s3_tables, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0,
     0, 0},
    {"create-options", 'a', "Include all MariaDB specific create options.",
    ...
    {"extended-insert", 'e',
     ...
     &extended_insert, &extended_insert, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0, 0,
     0},
    ...
    {"gtid", 0,
     ...
     "instead of the binlog file and offset; the file/offset will appear only "
     "as "
    ...
    {"ignore-database", OPT_IGNORE_DATABASE,
     "Do not dump the specified database. To specify more than one database "
     "to ignore, "
     "use the directive multiple times, once for each database. Only takes "
     "effect "
    ...
    {"lines-terminated-by", OPT_LTB,
     ...
     &lines_terminated, &lines_terminated, 0, GET_STR, REQUIRED_ARG, 0, 0, 0,
     0, 0, 0},
    {"lock-all-tables", 'x',
    ...
    {"log-queries", 0,
     "When restoring the dump, the server will, if logging turned on, log the "
     "queries to the general and slow query log.",
     &opt_logging, &opt_logging, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0, 0, 0},
    {"master-data", OPT_MASTER_DATA,
     "This causes the binary log position and filename to be appended to the "
     "output. If equal to 1, will print it as a CHANGE MASTER command; if "
     "equal"
     " to 2, that command will be prefixed with a comment symbol. "
    ...
    {"max_allowed_packet", OPT_MAX_ALLOWED_PACKET,
     ...
     REQUIRED_ARG, 24 * 1024 * 1024, 4096,
    ...
    {"no-data-med", 0,
     ...
     "Manage External Data (" MED_ENGINES ").",
     &opt_no_data_med, &opt_no_data_med, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0, 0,
     0},
    ...
    {"order-by-primary", OPT_ORDER_BY_PRIMARY,
     ...
     &opt_order_by_primary, &opt_order_by_primary, 0, GET_BOOL, NO_ARG, 0, 0,
     0, 0, 0, 0},
    {"password", 'p',
    ...
    */
    ...
    {"skip-opt", OPT_SKIP_OPTIMIZATION,
     ...
     "--create-options, --quick, --extended-insert, --lock-tables, "
     "--set-charset, and --disable-keys.",
     0, 0, 0, GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"socket", 'S', "The socket file to use for connection.",
     &opt_mysql_unix_port, &opt_mysql_unix_port, 0, GET_STR, REQUIRED_ARG, 0,
     0, 0, 0, 0, 0},
#include <sslopt-longopts.h>
    {"system", 256, "Dump system tables as portable SQL", &opt_system,
     &opt_system, &opt_system_types, GET_SET, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"tab", 'T',
     "Create tab-separated textfile for each table to given path. (Create "
     ".sql "
    ...
    {"triggers", OPT_TRIGGERS, "Dump triggers for each dumped table.",
     &opt_dump_triggers, &opt_dump_triggers, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0,
     0, 0},
    {"tz-utc", OPT_TZ_UTC,
...
static const char *load_default_groups[]= {"mysqldump",      "mariadb-dump",
...
static void write_header(FILE *sql_file, const char *db_name)
{
  ...
  {
    print_comment(
        sql_file, 0, "-- MariaDB dump %s  Distrib %s, for %s (%s)\n--\n",
        DUMP_VERSION, MYSQL_SERVER_VERSION, SYSTEM_TYPE, MACHINE_TYPE);
                              ...
                              const char *argument, const char *filename)
{
  ...
  case 'r':
    if (!(md_result_file=
              my_fopen(argument, O_WRONLY | FILE_BINARY, MYF(MY_WME))))
      exit(1);
    break;
  case 'W':
#ifdef _WIN32
...
#include <sslopt-case.h>
  ...
  case 'X':
    opt_xml= 1;
    extended_insert= opt_drop= opt_lock= opt_disable_keys= opt_autocommit=
        opt_create_db= 0;
    break;
  case 'i':
    opt_comments_used= 1;
  ...
  case (int) OPT_COMPATIBLE: {
    ...
    opt_compatible_mode= find_set(&compatible_mode_typelib, argument,
                                  strlen(argument), &err_ptr, &err_len);
    if (err_len)
    {
  ...
  case 'P':
    ...
    if (filename[0] == '\0' && !ignore_protocol_override &&
        protocol_to_force == MYSQL_PROTOCOL_DEFAULT)
    {
...
static int get_options(int *argc, char ***argv)
{
  ...
  if (my_hash_insert(&ignore_table,
      ...
      my_hash_insert(&ignore_table,
                     (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.schema",
                                         MYF(MY_WME))) ||
      my_hash_insert(&ignore_table,
                     (uchar *) my_strdup(PSI_NOT_INSTRUMENTED,
  ...
  /*
    Dumping under --system=stats with --replace or --inser-ignore is safe and
    will not retult into race condition. Otherwise dump only structure and
    ignore data by default while dumping.
  */
  ...
  if (opt_system & OPT_SYSTEM_USERS &&
      (my_hash_insert(&ignore_table,
                      (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.db",
                                          MYF(MY_WME))) ||
       my_hash_insert(&ignore_table,
                      (uchar *) my_strdup(PSI_NOT_INSTRUMENTED,
       ...
       my_hash_insert(&ignore_table,
                      (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.user",
                                          MYF(MY_WME))) ||
       my_hash_insert(&ignore_table,
                      (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.host",
                                          MYF(MY_WME))) ||
       my_hash_insert(&ignore_table, (uchar *) my_strdup(PSI_NOT_INSTRUMENTED,
                                                         "mysql.proxies_priv",
                                                         MYF(MY_WME))) ||
  ...
  if (opt_system & OPT_SYSTEM_PLUGINS &&
      my_hash_insert(&ignore_table,
                     (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.plugin",
                                         MYF(MY_WME))))
    return (EX_EOM);

  if (opt_system & OPT_SYSTEM_UDFS &&
      my_hash_insert(&ignore_table,
                     (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.func",
                                         MYF(MY_WME))))
    return (EX_EOM);

  if (opt_system & OPT_SYSTEM_SERVERS &&
      my_hash_insert(&ignore_table,
                     (uchar *) my_strdup(PSI_NOT_INSTRUMENTED, "mysql.servers",
                                         MYF(MY_WME))))
    return (EX_EOM);
  ...
  *mysql_params->p_net_buffer_length= opt_net_buffer_length;
  ...
  {
    fprintf(stderr,
            "%s: You can't use ..enclosed.. and ..optionally-enclosed.. at "
            "the same time.\n",
            my_progname_short);
    return (EX_USAGE);
  }
...
*/
static int check_consistent_binlog_pos(char *binlog_pos_file,
                                       char *binlog_pos_offset)
{
  MYSQL_RES *res;
  MYSQL_ROW row;
  int found;
...
*/
static int get_binlog_gtid_pos(char *binlog_pos_file, char *binlog_pos_offset,
                               char *out_gtid_pos)
{
  ...
  if (len_pos_file >= FN_REFLEN || len_pos_offset > LONGLONG_LEN)
    return 0;
  mysql_real_escape_string(mysql, file_buf, binlog_pos_file,
                           (ulong) len_pos_file);
  mysql_real_escape_string(mysql, offset_buf, binlog_pos_offset,
                           (ulong) len_pos_offset);
...
*/
static int get_gtid_pos(char *out_gtid_pos, int master)
{
  MYSQL_RES *res;
  MYSQL_ROW row;
  int found;
                         ...
                         uint token_len)
{
  ...
  uint status= my_ci_instr(&my_charset_latin1, str, str_len, token, token_len,
                           &match, 1);
                               ...
                               int *db_cl_altered)
{
  if (strcmp(current_db_cl_name, required_db_cl_name) != 0)
  {
    ...
    CHARSET_INFO *db_cl=
        get_charset_by_name(required_db_cl_name, MYF(MY_UTF8_IS_UTF8MB3));

    if (!db_cl)
      return 1;

    fprintf(sql_file, "ALTER DATABASE %s CHARACTER SET %s COLLATE %s %s\n",
            (const char *) quoted_db_name, (const char *) db_cl->cs_name.str,
            (const char *) db_cl->coll_name.str, (const char *) delimiter);
                                ...
                                const char *delimiter, const char *db_cl_name)
{
  ...
  CHARSET_INFO *db_cl=
      get_charset_by_name(db_cl_name, MYF(MY_UTF8_IS_UTF8MB3));
...
static int switch_character_set_results(MYSQL *mysql, const char *cs_name)
{
  ...
  query_length= my_snprintf(query_buffer, sizeof(query_buffer),
...
static char *cover_definer_clause(const char *stmt_str, size_t stmt_length,
                                  ...
                                  const char *keyword_str, uint keyword_length)
{
  char *definer_begin=
...
static int connect_to_db(char *host, char *user, char *passwd)
{
...
#endif
  if (opt_protocol)
    mysql_options(&mysql_connection, MYSQL_OPT_PROTOCOL,
                  (char *) &opt_protocol);
  ...
  if (!mysql_real_connect(&mysql_connection, host, user, passwd, NULL,
  ...
  for (i= 0; (field= mysql_fetch_field(tableRes)); i++)
  {
    if ((*row)[i])
    {
      ...
      else
      {
        ...
        fputs("=\"", xml_file);
        if (opt_copy_s3_tables && !strcmp(field->name, "Engine") &&
            !strcmp((*row)[i], "S3"))
                              ...
                              int delimiter_max_size)
{
       ...
       delimiter_max_size++)
  {
    ...
    if (presence == NULL)
    { /* the proposed delimiter is not in the query. */
...
{
  char query_buff[QUERY_LENGTH];
  const char *routine_type[]= {"FUNCTION", "PROCEDURE", "PACKAGE",
                               "PACKAGE BODY"};
  ...
  */
  ...
  for (i= 0; i < upper_bound; i++)
  {
    my_snprintf(query_buff, sizeof(query_buff),
                "SHOW %s STATUS WHERE Db = '%s'", routine_type[i],
                db_name_buff);
...
/*
  get_sequence_structure-- retrievs sequence structure, prints out
  corresponding CREATE statement ARGS seq         - sequence name db          -
  db name
*/
...
static void get_sequence_structure(const char *seq, const char *db)
{
  ...
  if (!opt_no_create_info)
  {
    ...
    my_snprintf(buff, sizeof(buff), "SELECT next_not_cached_value FROM %s",
                result_seq);
    if (mysql_query_with_error_report(mysql, &result, buff))
    {
      DBUG_VOID_RETURN;
...
static uint get_table_structure(const char *table, const char *db,
      ...
      while ((row= mysql_fetch_row(result)))
      {
        ...
        if (atoi(row[3]) == 1)
        {
          ...
          if (atoi(row[1])) /* Test if duplicate key */
            ...
            fputs(",\n  PRIMARY KEY (",
                  sql_file); /* First UNIQUE is primary */
          else
            fprintf(sql_file, ",\n  UNIQUE %s (",
                    quote_name(row[2], name_buff, 0));
        }
                       ...
                       const uchar *hash_key, size_t len)
{
  ...
  */
  ...
  {
    verbose_msg(
        "-- Skipping data table mysql.event, --skip-events was used\n");
    DBUG_VOID_RETURN;
  }
  ...
  else
  {
            ...
            if (length)
            {
              if (!(field->flags & NUM_FLAG))
              {
                ...
                */
                ...
                {
                  dynstr_append_checked(&extended_row, "'");
                  extended_row.length+= mysql_real_escape_string(
                      &mysql_connection,
                      &extended_row.str[extended_row.length], row[i], length);
    ...
    if (mysql_errno(mysql))
    {
      my_snprintf(buf, sizeof(buf),
                  ...
                  my_progname_short, mysql_errno(mysql), mysql_error(mysql),
                  result_table, rownr);
      fputs(buf, stderr);
      error= EX_CONSCHECK;
      goto err;
...
static int dump_grants(const char *user_role)
{
  ...
  {
    if (strncmp(row[0], "SET DEFAULT ROLE", sizeof("SET DEFAULT ROLE") - 1) ==
        0)
      continue;
...
static int dump_all_users_roles_and_grants()
{
  ...
  my_bool mysql_roles_exist=
      (mysql_get_server_version(mysql) >= 80001) && !maria_roles_exist;
  ...
  */
  ...
  /* Roles, with user admins first, then roles they administer, and recurse on
   * that */
  ...
  */
  if (mysql_roles_exist &&
      mysql_query_with_error_report(
          ...
          "SELECT CONCAT(QUOTE(FROM_USER), '/*!80001 @', QUOTE(FROM_HOST), "
          "'*/') AS r,"
  ...
  while ((row= mysql_fetch_row(tableres)))
  {
    /* MySQL-8.0 export capability */
    if (opt_replace_into)
      fprintf(md_result_file, "/*!80001 DROP ROLE IF EXISTS %s */;\n", row[0]);
    fprintf(md_result_file, "/*!80001 CREATE ROLE %s%s */;\n",
            opt_ignore ? "IF NOT EXISTS " : "", row[0]);
    /* By default created with current role */
    fprintf(md_result_file,
            "%sROLE %s%s WITH ADMIN mariadb_dump_import_role */;\n",
            opt_replace_into ? "/*M!100103 CREATE OR REPLACE "
                             : "/*M!100005 CREATE ",
            opt_ignore ? "IF NOT EXISTS " : "", row[0]);
  ...
  if (mysql_roles_exist &&
      mysql_query_with_error_report(
          mysql, &tableres,
          "SELECT IF(DEFAULT_ROLE_HOST IS NULL, 'NONE', "
          "CONCAT(QUOTE(DEFAULT_ROLE_USER),"
          "                                                    '@', "
          "QUOTE(DEFAULT_ROLE_HOST))) as r,"
          "  CONCAT(QUOTE(mu.USER),'@',QUOTE(mu.HOST)) as u "
          "FROM mysql.user mu LEFT JOIN mysql.default_roles using (USER, "
          "HOST)"))
    return 1;
  while ((row= mysql_fetch_row(tableres)))
  {
  ...
  if (maria_roles_exist &&
      mysql_query_with_error_report(mysql, &tableres,
                                    ...
                                    "   ORDER BY m.role"))
    return 1;
  if (mysql_roles_exist && mysql_query_with_error_report(
                               mysql, &tableres,
                               "SELECT DISTINCT CONCAT(QUOTE(FROM_USER),'@', "
                               "QUOTE(FROM_HOST)) AS r "
...
static int dump_all_plugins()
{
  ...
  while ((row= mysql_fetch_row(tableres)))
  {
    ...
    fprintf(md_result_file, "INSTALL PLUGIN %s %s SONAME '%s';\n", row[0],
...
static int dump_all_udfs()
{
  ...
  static const char *udf_types[]= {"STRING", "REAL",    "INT",
                                   "ROW",    "DECIMAL", "TIME"};
  MYSQL_ROW row;
  MYSQL_RES *tableres;
  ...
  while ((row= mysql_fetch_row(tableres)))
  {
    ...
    fprintf(md_result_file,
            ...
            opt_ignore ? "IF NOT EXISTS " : "", row[0], udf_types[retresult],
            row[2]);
  }
  mysql_free_result(tableres);
...
static int dump_all_servers()
{
  ...
  {
    fprintf(md_result_file,
            "CREATE %sSERVER %s%s FOREIGN DATA WRAPPER %s OPTIONS (",
            opt_replace_into ? "/*M!100103 OR REPLACE */ " : "",
    ...
    for (i= 1; i < num_fields; i++)
    {
      ...
      qstring=
          (f->type == MYSQL_TYPE_STRING || f->type == MYSQL_TYPE_VAR_STRING)
              ? "'"
              : "";
      fprintf(md_result_file, "%s%s %s%s%s", (comma_prepend ? ", " : ""),
              f->name, qstring, row[i], qstring);
      comma_prepend= 1;
    }
    fputs(");\n", md_result_file);
  }
                                       ...
                                       int tables)
{
  ...
  init_dynamic_string_checked(&dynamic_where,
  ...
  for (i= 0; i < tables; i++)
  {
    mysql_real_escape_string(mysql, name_buff, table_names[i],
...
static int dump_tablespaces_for_databases(char **databases)
{
  ...
  for (i= 0; databases[i] != NULL; i++)
  {
    ...
    mysql_real_escape_string(mysql, db_name_buff, databases[i],
...
static int dump_tablespaces(char *ts_where)
{
  ...
  */
  ...
  if (ts_where)
  {
    dynstr_append_checked(&sqlbuf, " AND LOGFILE_GROUP_NAME IN ("
  ...
  dynstr_append_checked(&sqlbuf, " GROUP BY LOGFILE_GROUP_NAME, FILE_NAME"
...
{
  return !my_hash_search(&ignore_database, (uchar *) hash_key,
                         strlen(hash_key));
}
...
int init_dumping_tables(char *qdatabase)
{
  ...
  if (!opt_create_db)
  {
    ...
    my_snprintf(qbuf, sizeof(qbuf), "SHOW CREATE DATABASE IF NOT EXISTS %s",
                qdatabase);
...
static int dump_all_tables_in_db(char *database)
{
  ...
  *afterdot++= '.';
    ...
    if (include_table((uchar *) hash_key, end - hash_key))
    {
      ...
      */
      if (opt_single_transaction && mysql_get_server_version(mysql) >= 50500)
      {
        ...
        if (mysql_query_with_error_report(mysql, 0,
  ...
  if (using_mysql_db)
  {
    ...
    {
      if (!get_table_structure((char *) "transaction_registry", database,
                               table_type, &ignore_flag))
        verbose_msg(
            "-- Warning: get_table_structure() failed with some internal "
...
static char *get_actual_table_name(const char *old_table_name,
                                   int lower_case_table_names, MEM_ROOT *root)
{
  char *name= 0;
...
static int dump_selected_tables(char *db, char **table_names, int tables)
{
  ...
  if (!(dump_tables= pos=
            (char **) alloc_root(&glob_root, tables * sizeof(char *))))
...
static int do_stop_slave_sql(MYSQL *mysql_con)
{
  ...
  if (mysql_query_with_error_report(mysql_con, &slave,
                                    multi_source ? "SHOW ALL SLAVES STATUS"
                                                 : "SHOW SLAVE STATUS"))
    return (1);
  ...
  */
  if ((mysql_get_server_version(mysql_con) < 40100) && opt_master_data)
  {
    fprintf(stderr,
            ...
            ignore_errors ? "Continuing due to --force, backup may not be "
...
char check_if_ignore_table(const char *table_name, char *table_type)
{
  ...
  else
  {
    ...
    */
    ...
    if (opt_delayed)
    {
      if (strcmp(table_type, "MyISAM") && strcmp(table_type, "ISAM") &&
          strcmp(table_type, "ARCHIVE") && strcmp(table_type, "HEAP") &&
...
static char *primary_key_fields(const char *table_name)
{
  ...
  my_snprintf(show_keys_buff, sizeof(show_keys_buff), "SHOW KEYS FROM %s",
              table_name);
...
static int replace(DYNAMIC_STRING *ds_str, const char *search_str,
                   ulong search_len, const char *replace_str,
                   ulong replace_len)
{
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if (opt_master_data &&
      do_show_master_status(mysql, consistent_binlog_pos, have_mariadb_gtid,
                            opt_use_gtid))
    goto err;
  if (opt_slave_data &&
      do_show_slave_status(mysql, opt_use_gtid, have_mariadb_gtid))
    goto err;
